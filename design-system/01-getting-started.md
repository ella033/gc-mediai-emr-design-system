...
import { CEDSProvider } from '@cloud-emr/design-system';

export default function RootLayout({ children }) {
  return (
    <html lang="ko">
      <body>
        <CEDSProvider theme="light">
          {children}
        </CEDSProvider>
      </body>
    </html>
...
// 시스템 설정 따르기
<CEDSProvider theme="system">{children}</CEDSProvider>

// 라이트 모드 고정
<CEDSProvider theme="light">{children}</CEDSProvider>

// 다크 모드 고정
<CEDSProvider theme="dark">{children}</CEDSProvider>
```
...
.custom-element {
  ...
  border-radius: var(--ceds-radius-md);
  padding: var(--ceds-spacing-8);
  font-size: var(--ceds-font-size-body1);
}
```
...
│   ├── layout.tsx          # 글로벌 레이아웃 + CEDSProvider
│   ├── page.tsx            # 메인 대시보드
│   └── chart/              # 진료 화면
│       └── [patientId]/
│           └── page.tsx
